		...
		internal static void DefineItemWeaponGroups1( IList<EntityGroupBuilderDefinition<Item>> defs ) {
			...
			defs.Add( new EntityGroupBuilderDefinition<Item>(
				...
				matcher: new ItemGroupMatcher( ( item, grps ) => {
					...
					var projPool = ModContent.GetInstance<EntityGroups>().GetProjPool();
			...
			defs.Add( new EntityGroupBuilderDefinition<Item>(
				...
				matcher: new ItemGroupMatcher( ( item, grps ) => {
					...
					bool isCraftedWith = RecipeFinderLibraries.RecipeExists_Cached(
						createItemTypes: new HashSet<int> { item.type },
						allIngredients: null,
						anyIngredients: new Dictionary<int, (int, int)>{ { ItemID.SpellTome, (1, 1000) } }
					);
